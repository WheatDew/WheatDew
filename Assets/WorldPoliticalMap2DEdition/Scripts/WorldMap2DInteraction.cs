...
namespace WPMF {
	...
	public partial class WorldMap2D : MonoBehaviour {
		...
		[SerializeField]
		Vector2
			_flyToScreenCenter = Misc.Vector2half;
		...
		[SerializeField]
		bool
			_zoomOnCursorPosition = true;

		public bool zoomOnCursorPosition {
			get { return _zoomOnCursorPosition; }
			set {
				if (_zoomOnCursorPosition != value) {
					_zoomOnCursorPosition = value;
					isDirty = true;
				}
			}
		}
		...
		/// <summary>
		/// If set to true, mouse interactions will be disabled when pointer is over UI
		/// </summary>
		...
		public void CenterMap () {
			...
			SetDestinationAndDistance (Misc.Vector2zero, 0f, distance);
//			if (_staticCamera) {
...
//			}
		...
		/// <summary>
		/// Starts navigation to target location in local 2D coordinates with duration and zoomLevel
		/// </summary>
		...
		/// Starts navigation to target location in local 2D coordinates with duration and zoomLevel
		/// </summary>
		public void FlyToLocation(float x, float y, float duration, float zoomLevel) {
			SetDestination (new Vector2(x,y), duration, zoomLevel);
		}


		/// <summary>
		/// Starts navigation to latitude/longitude with duration and zoomLevel
		/// </summary>
		public void FlyToLatLon(float lat, float lon, float duration, float zoomLevel) {
			Vector2 dest = Conversion.GetLocalPositionFromLatLon (new Vector2 (lat, lon));
			SetDestination (dest, duration, zoomLevel);
		}
