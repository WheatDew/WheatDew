...
namespace WPMF {
	...
	public class WorldMap2D_Ticker : MonoBehaviour {
		...
		public void Init () {
			...
			if (defaultFont == null) {
				defaultFont = Instantiate(Resources.Load <Font> ("WPMF/Font/Lato"));
		...
		public bool UpdateTickerBands () {
			...
			Camera mainCamera = map.mainCamera;
			if (tickerBaseLayer == null && mainCamera != null) {
				changes = true;
				GameObject overlayLayer = overlayMode ? map.mainCamera.gameObject : map.GetOverlayLayer (true);
				if (overlayLayer == null)
					return changes;
				Transform t = overlayLayer.transform.Find ("TickersLayer");
				if (t == null) {
					...
					if (_overlayMode) {
						float z = mainCamera.nearClipPlane + 0.0001f;
						tickerBaseLayer.transform.localPosition = new Vector3 (0, 0, z);
						Vector3 pos0 = mainCamera.transform.InverseTransformPoint (mainCamera.ScreenToWorldPoint (new Vector3 (0, 0, z)));
						Vector3 pos1 = mainCamera.transform.InverseTransformPoint (mainCamera.ScreenToWorldPoint (new Vector3 (Camera.main.pixelWidth, Camera.main.pixelHeight, z)));
		...
		public void AddTickerText (TickerText tickerText) {
			...
			GameObject tickerTextObj = Drawing.CreateText (tickerText.text, null, pos, customFont, tickerText.textColor, tickerText.drawTextShadow, customShadowMaterial, tickerText.shadowColor, tickerText.anchor);
