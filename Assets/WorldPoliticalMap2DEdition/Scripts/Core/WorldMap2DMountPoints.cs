...
namespace WPMF {
	...
	public partial class WorldMap2D : MonoBehaviour {
		...
		void ReadMountPointsPackedString () {
			string mountPointsCatalogFileName = "WPMF/Geodata/mountPoints";
		...
		void ReadMountPointsPackedString (string s) {
			...
			for (int k=0; k<mountPointsCount; k++) {
				...
				if (countryIndex>=0) {
					...
					int type = int.Parse (mountPointInfo [3], invariantCulture);
					float x = float.Parse (mountPointInfo [4], invariantCulture);
					float y = float.Parse (mountPointInfo [5], invariantCulture);
	...
	#region Drawing stuff
		...
		public void DrawMountPoints() {
			...
			mountPointsLayer.hideFlags = HideFlags.DontSaveInEditor;
			...
			for (int k=0; k<mountPoints.Count; k++) {
				...
				mpObj.hideFlags = HideFlags.DontSaveInEditor | HideFlags.HideInHierarchy;
