...
namespace WPMF {
	public static class Drawing {
		...
		public static GameObject CreateSurface (string name, Vector3[] points, int[] indices, Material material, Rect rect, Vector2 textureScale, Vector2 textureOffset, float textureRotation) {
			...
			hexa.hideFlags = HideFlags.DontSaveInEditor | HideFlags.HideInHierarchy;
			Mesh mesh = new Mesh ();
		...
		public static GameObject CreateText (string text, GameObject parent, Vector2 center, Font labelFont, Color textColor, bool showShadow, Material shadowMaterial, Color shadowColor, TextAnchor anchor = TextAnchor.MiddleCenter) {
			// create base text
			GameObject textObj = new GameObject (text);
			textObj.hideFlags = HideFlags.DontSaveInEditor | HideFlags.HideInHierarchy;
			if (parent != null) {
			...
			// add shadow
			if (showShadow) {
				GameObject shadow = UnityEngine.Object.Instantiate(textObj);
				shadow.hideFlags = HideFlags.DontSaveInEditor | HideFlags.HideInHierarchy;
				shadow.name = "shadow";
