...
namespace WPMF
{
	public partial class WorldMap2D : MonoBehaviour
	{
		...
		const string PROVINCES_ROOT = "Provinces";
		...
		bool DrawProvinces (List<Country>targetCountries, bool forceRefresh)
		{
			...
			if (frontiersPoints == null) {
				frontiersPoints = new List<Vector3> (290000);
			} else {
				frontiersPoints.Clear ();
			}
			if (frontiersCacheHit == null) {
				frontiersCacheHit = new Dictionary<double, Region> (145000);
			} else {
			...
			provincesObj = new GameObject (PROVINCES_ROOT);
			provincesObj.hideFlags = HideFlags.DontSaveInEditor;
			...
			if (_enableProvinceHighlight) {
				if (existsInCache) {
					...
					if (provinceRegionHighlightedObj != null) {
						...
						Renderer r = provinceRegionHighlightedObj.GetComponent <Renderer> ();
						Material surfMat = r.sharedMaterial;
						hudMatProvince.mainTexture = null;
						if (surfMat != hudMatProvince) {
							if (surfMat.mainTexture != null) {
								hudMatProvince.mainTexture = surfMat.mainTexture;
							}
