...
namespace WPMF {
    ...
    public partial class WorldMap2D : MonoBehaviour {
        ...
        public int GetProvinceIndex(Province province) {
            int provIndex;
            if (provinceLookup.TryGetValue(province, out provIndex)) {
                return provIndex;
            } else {
                return -1;
            }
                ...
                for (int k = 0; k < hits.Length; k++) {
                    if (hits[k].collider.gameObject == gameObject) {
                        ...
                        for (int c = 0; c < provinces.Length; c++) {
                            if (provinces[c].regionsRect2D.Contains(localHit)) {
                                if (provinces[c].regions != null) {
                                    int regCount = provinces[c].regions.Count;
        ...
        /// </summary>
        public bool ToggleProvinceSurface(string name, bool visible, Color color = default(Color)) {
            for (int c = 0; c < provinces.Length; c++) {
        ...
        public GameObject ToggleProvinceRegionSurface(int provinceIndex, int regionIndex, bool visible, Color color) {
            ...
            int cacheIndex = GetCacheIndexForProvinceRegion(provinceIndex, regionIndex);
            GameObject surf;
            if (surfaces.TryGetValue(cacheIndex, out surf) && surf != null) {
                ...
                if (_provinceHighlightedIndex != provinceIndex || _provinceRegionHighlightedIndex != regionIndex || !_enableProvinceHighlight) {
                    surf.GetComponent<Renderer>().sharedMaterial.color = color;
                }
            } else {
                return GenerateProvinceRegionSurface(provinceIndex, regionIndex, coloredMat);
            }
        ...
        public void HideProvinceRegionSurface(int provinceIndex, int regionIndex) {
            ...
            _provinces[provinceIndex].regions[regionIndex].customMaterial = null;
        }
        ...
        public void SetProvinceRegionMaterial(int provinceIndex, int regionIndex, Material material) {
            GameObject surf = ToggleProvinceRegionSurface(provinceIndex, regionIndex, true, Color.white);
            if (surf == null) return;
