...
//			}
		...
		public void CountryRegionTransferTo ()
		{
			...
			if (sourceCountry.regions.Count == 1) {
				map.CountryDelete(countryIndex, true);
				if (countryIndex < targetCountryIndex) {
					targetCountryIndex--;
				}
			} else {
			...
			for (int k = 0; k < entities2.Count; k++) {
				...
				for (int r = 0; r < entity.regions.Count; r++) {
					...
					for (int p = 0; p < region.points.Length; p++) {
						...
						sb.Append (point.x.ToString (invariantCulture) + ",");
						sb.Append (point.y.ToString (invariantCulture));
					}
				}
			}
			return sb.ToString ();
