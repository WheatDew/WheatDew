...
//					}
		...
		/// <summary>
		/// Allow country names to include spaces to spread across big country surfaces
		/// </summary>
		[SerializeField]
		bool
			_countryLabelSpread = true;

		public bool countryLabelSpread {
			get {
				return _countryLabelSpread;
			}
			set {
				if (_countryLabelSpread != value) {
					_countryLabelSpread = value;
					RedrawMapLabels();
					isDirty = true;
				}
			}
		...
		public int GetCountryIndex (string countryName) {
			int countryIndex;
			if (countryLookup.TryGetValue (countryName, out countryIndex))
				return countryIndex;
			else
				return -1;
		}
		...
		/// </summary>
		public bool ToggleCountrySurface (string name, bool visible, Color color = default(Color)) {
			int countryIndex = GetCountryIndex (name);
			if (countryIndex >= 0) {
		...
		/// </summary>
		public void ToggleContinentSurface (string continentName, bool visible, Color color = default(Color)) {
		...
		/// <summary>
		/// Uncolorize/hide specified country by name in the countries collection.
		/// </summary>
		public void HideCountrySurface (string countryName) {
		...
		/// <summary>
		/// Colorize main region of a country by name in the countries collection.
		/// </summary>
		public GameObject ToggleCountryMainRegionSurface (string countryName, bool visible, Color color) {
			int countryIndex = GetCountryIndex (countryName);
		...
		public GameObject ToggleCountryMainRegionSurface (int countryIndex, bool visible, Texture2D texture) {
			return ToggleCountryRegionSurface (countryIndex, countries [countryIndex].mainRegionIndex, visible, Color.white, texture, Misc.Vector2one, Misc.Vector2zero, 0);
		}
		...
		public GameObject ToggleCountryMainRegionSurface (int countryIndex, bool visible, Color color, Texture2D texture, Vector2 textureScale, Vector2 textureOffset, float textureRotation) {
			return ToggleCountryRegionSurface (countryIndex, countries [countryIndex].mainRegionIndex, visible, color, texture, textureScale, textureOffset, textureRotation);
		}
		...
		public GameObject ToggleCountryRegionSurface (int countryIndex, int regionIndex, bool visible, Color color, Texture2D texture, Vector2 textureScale, Vector2 textureOffset, float textureRotation) {
			...
			if (region.customMaterial != null && isHighlighted && region.customMaterial.color != hudMatCountry.color) {
				ApplyMaterialToSurface (surf, hudMatCountry);
		...
		/// <summary>
        /// Overrides the material used by a country region
        /// </summary>
        /// <param name="countryIndex"></param>
        /// <param name="regionIndex"></param>
        /// <param name="material"></param>
		public void SetCountryRegionMaterial(int countryIndex, int regionIndex, Material material) {
			GameObject surf = ToggleCountryRegionSurface(countryIndex, regionIndex, true, Color.white);
			if (surf == null) return;
