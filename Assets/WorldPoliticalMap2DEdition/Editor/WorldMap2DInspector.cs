...
namespace WPMF
{
	...
	public class WorldMap2DInspector : Editor
	{
		...
		public override void OnInspectorGUI ()
		{
			...
			EditorGUILayout.BeginHorizontal ();
			GUILayout.Label (new GUIContent ("Screen Center", "The assumed screen center defined in normalized coordinates 0-1 range (0.5, 0.5 = normal screen center). Customize to offset FlyTo() destination for example."), GUILayout.Width (120));
			_map.flyToScreenCenter = EditorGUILayout.Vector2Field ("", _map.flyToScreenCenter);
			EditorGUILayout.EndHorizontal ();
...
//												GUIStyle labelStyle = _map.showEarth ? sectionHeaderBoldStyle : sectionHeaderNormalStyle;
			...
			if (expandEarthSection) {
                ...
                EditorGUILayout.BeginHorizontal();
                GUILayout.Label("Sun Light", GUILayout.Width(120));
...
//												labelStyle = _map.showCities ? sectionHeaderBoldStyle : sectionHeaderNormalStyle;
			...
			if (expandCitiesSection) {
				...
				if (_map.showCities) {
					...
					EditorGUILayout.EndHorizontal ();
					EditorGUILayout.BeginHorizontal ();
					GUILayout.Label ("   Region Capitals", GUILayout.Width (120));
					_map.citiesRegionCapitalColor = EditorGUILayout.ColorField (_map.citiesRegionCapitalColor);
					EditorGUILayout.EndHorizontal ();
					EditorGUILayout.BeginHorizontal ();
					GUILayout.Label ("   Country Capitals", GUILayout.Width (120));
...
//												labelStyle = _map.showCountryNames || _map.showFrontiers ? sectionHeaderBoldStyle : sectionHeaderNormalStyle;
			...
			if (expandCountriesSection) {
				...
				if (_map.showFrontiers) {
					...
					_map.frontiersColorOuter = EditorGUILayout.ColorField (_map.frontiersColorOuter);
					EditorGUILayout.EndHorizontal ();
				...
				if (_map.showCountryNames) {
					...
					EditorGUILayout.BeginHorizontal();
					GUILayout.Label("  Spread Text", GUILayout.Width(120));
					_map.countryLabelSpread = EditorGUILayout.Toggle(_map.countryLabelSpread);
					EditorGUILayout.EndHorizontal();
			...
			if (expandInteractionSection) {
				...
				if (_map.allowUserDrag) {
					...
					EditorGUILayout.EndHorizontal ();
					EditorGUILayout.BeginHorizontal ();
					GUILayout.Label ("   Constant Speed", GUILayout.Width (120));
					...
					if (_map.allowScrollOnScreenEdges) {
						EditorGUILayout.BeginHorizontal ();
						GUILayout.Label ("      Edge Thickness", GUILayout.Width (120));
				...
				if (_map.allowUserZoom) {
					EditorGUILayout.BeginHorizontal();
					GUILayout.Label("   Zoom On Cursor", GUILayout.Width(120));
					_map.zoomOnCursorPosition = EditorGUILayout.Toggle(_map.zoomOnCursorPosition);
					EditorGUILayout.EndHorizontal();
					EditorGUILayout.BeginHorizontal ();
					GUILayout.Label ("   Speed", GUILayout.Width (120));
			...
			if (_map.gameObject.activeInHierarchy) {
				...
				if (_map.gameObject.GetComponent<WorldMap2D_Calculator> () == null) {
					if (GUILayout.Button ("Open Calculator")) {
						_map.gameObject.AddComponent<WorldMap2D_Calculator> ();
					}
				} else {
					if (GUILayout.Button ("Hide Calculator")) {
				...
				if (_map.gameObject.GetComponent<WorldMap2D_Ticker> () == null) {
					if (GUILayout.Button ("Open Ticker")) {
						_map.gameObject.AddComponent<WorldMap2D_Ticker> ();
					}
				} else {
					if (GUILayout.Button ("Hide Ticker")) {
				...
				if (_map.gameObject.GetComponent<WorldMap2D_Decorator> () == null) {
					if (GUILayout.Button ("Open Decorator")) {
						_map.gameObject.AddComponent<WorldMap2D_Decorator> ();
					}
				} else {
					if (GUILayout.Button ("Hide Decorator")) {
