...
namespace WPMF
{
	...
	public class WorldMap2D_EditorInspector : Editor
	{
		...
		void OnEnable ()
		{
			...
			icons [0] = Resources.Load<Texture2D> ("WPMF/IconSelect");
			icons [1] = Resources.Load<Texture2D> ("WPMF/IconPolygon");
			icons [2] = Resources.Load<Texture2D> ("WPMF/IconUndo");
			icons [3] = Resources.Load<Texture2D> ("WPMF/IconConfirm");
			icons [4] = Resources.Load<Texture2D> ("WPMF/IconPoint");
			icons [5] = Resources.Load<Texture2D> ("WPMF/IconCircle");
			icons [6] = Resources.Load<Texture2D> ("WPMF/IconMagnet");
			icons [7] = Resources.Load<Texture2D> ("WPMF/IconSplitVert");
			icons [8] = Resources.Load<Texture2D> ("WPMF/IconSplitHoriz");
			icons [9] = Resources.Load<Texture2D> ("WPMF/IconDelete");
			icons [10] = Resources.Load<Texture2D> ("WPMF/IconEraser");
			icons [11] = Resources.Load<Texture2D> ("WPMF/IconMorePoints");
			icons [12] = Resources.Load<Texture2D> ("WPMF/IconCreate");
			icons [13] = Resources.Load<Texture2D> ("WPMF/IconPenCountry");
			icons [14] = Resources.Load<Texture2D> ("WPMF/IconTarget");
			icons [15] = Resources.Load<Texture2D> ("WPMF/IconPenCountryRegion");
			icons [16] = Resources.Load<Texture2D> ("WPMF/IconPenProvince");
			icons [17] = Resources.Load<Texture2D> ("WPMF/IconPenProvinceRegion");
			icons [18] = Resources.Load<Texture2D> ("WPMF/IconMove");
			icons [19] = Resources.Load<Texture2D> ("WPMF/IconMountPoint");
		...
		void ShowEntitySelectors ()
		{
			...
			if (_editor.editingMode == EDITING_MODE.PROVINCES) {
				...
				if (_editor.provinceIndex >= 0) {
					...
					EditorGUILayout.BeginHorizontal ();
					GUILayout.Label ("   Merge With", GUILayout.Width (90));
					_editor.GUIProvinceTransferToProvinceIndex = EditorGUILayout.Popup (_editor.GUIProvinceTransferToProvinceIndex, provinceNames);
		...
		[MenuItem("CONTEXT/WorldMap2D_Editor/Fix Orphan Cities")]
		...
		static void RestoreBackup (MenuCommand command)
		{
			...
			// Countries110
			string fullFileName = backupFolder + "/countries110.txt";
			...
			// Mount points
			fullFileName = backupFolder + "/mountPoints.txt";
			if (File.Exists (fullFileName)) {
				AssetDatabase.DeleteAsset (geoDataFolder + "/mountPoints.txt");
				AssetDatabase.SaveAssets ();
				AssetDatabase.CopyAsset (fullFileName, geoDataFolder + "/mountPoints.txt");
			}
		...
		// Add a menu item called "Double Mass" to a Rigidbody's context menu.
		[MenuItem ("CONTEXT/WorldMap2D_Editor/Create Low Definition Geodata File")]
		...
		void DrawEditorProvinceNames ()
		{
			...
			for (int p = 0; p < _editor.highlightedRegions.Count; p++) {
				...
				if (region.regionIndex == region.entity.mainRegionIndex) {
					if (!string.IsNullOrEmpty(region.entity.name)) {
